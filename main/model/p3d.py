...
def P3D199(pretrained=False,modality='RGB', num_classes=400,**kwargs):
    """construct a P3D199 model based on a ResNet-152-3D model.
    """
    model = P3D(Bottleneck, [3, 8, 36, 3], modality=modality,**kwargs)
    if pretrained==True:
        if modality=='RGB':
            pretrained_file='model/pretrained/p3d_rgb_199.checkpoint.pth.tar'
        elif modality=='Flow':
            pretrained_file='model/pretrained/p3d_flow_199.checkpoint.pth.tar'
